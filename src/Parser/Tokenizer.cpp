...
bool Tokenizer::isAllowedChar(char c) const {
    if (std::isalnum(static_cast<unsigned char>(c))) return true;
    if (std::isspace(static_cast<unsigned char>(c))) return true;
    if (m_syn.isPipe(c) || m_syn.isRedir(c) || m_syn.isDash(c) || m_syn.isQuote(c)) return true;
...
void Tokenizer::checkForLexicalErrors(const std::string &line) const {
    std::vector<int> errorPositions;
    bool inQuotes = false;

    // skeniranje linije
    for (int i = 0; i < (int)line.size(); ++i) {
        if (line[i] == m_syn.quote) {
            inQuotes = !inQuotes;
        }
        else if (!inQuotes && !isAllowedChar(line[i])) {
            errorPositions.push_back(i);
        }
    }

    if (!errorPositions.empty()) {
        std::string markerLine(line.size(), ' '); // prazna linija iste duzine
        for (int pos : errorPositions) {
            markerLine[pos] = '^';
        }

        std::string errorMessage = "Lexical Error: unexpected characters:\n" + line + "\n" + markerLine;
        throw Exception(ErrorType::Lexical, errorMessage);
    }
}
